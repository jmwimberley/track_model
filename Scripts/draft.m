...
%% Indexing method of generating centreline (whichever path is bigger takes lead, e.g for Assen)
...
lowest_alt = min([OuterXYZ(:,3);InnerXYZ(:,3)]);
highest_alt = max([OuterXYZ(:,3);InnerXYZ(:,3)]);
...
for jj=2:length(xyzC)
    ...
    sag_curvature(jj) = ((slope_angle_r(jj)-slope_angle_r(jj-1))/((dist_change(jj)-dist_change(jj-1))))/sqrt(((xyzC(jj,1)-xyzC(jj-1,1))/(dist_change(jj)-dist_change(jj-1)))^2+((xyzC(jj,3)-xyzC(jj-1,3))/(dist_change(jj)-dist_change(jj-1)))^2);
end
...
%% 2D PLOTS
figure
plot(centre_distance_m,centre_slope_angle_d,'LineWidth',2,'Color','k')
xlim([min(centre_distance_m) max(centre_distance_m)])
ylim([min(centre_slope_angle_d)-5 max(centre_slope_angle_d)+5])
title('Distance Vs Slope Angle of Assen')
xlabel('Distance (m)')
...
figure
plot(centre_distance_m,CentreXYZ(:,3),'LineWidth',2,'Color','k')
xlim([min(centre_distance_m) max(centre_distance_m)])
%ylim([min(CentreXYZ(:,3))-5 max(CentreXYZ(:,3))+5])
title('Distance Vs Relative Altitude of Assen')
xlabel('Distance (m)')
...
%% LOOPS TO GENERATE MULTIPLE LINES BETWEEN INNER AND OUTER
splines = cell(length(xyzC),1);